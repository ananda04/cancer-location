%% convultion denoising algorithm
...
weightgm = 0*normavggraym


gauscan = max(weightc+normrnd(0, 0.01),0)
gauswm = max(weightwm+normrnd(0, 0.01),0)
gausgm = max(weightgm+normrnd(0, 0.01),0)
...
l = length(i1)
for m = 1:l
    r  = cat(2, r, (i1(:,m).*gauscan+j1(:,m).*gauswm+k1(:,m).*gausgm))
end 
for m = 1:l
    s = cat(2, r, (i1(:,m).*weightc+j1(:,m).*weightwm+k1(:,m).*weightgm))
end 
M = conv2(r,s)
...
for b = 1:l
    t1 = [t1,sum(gauswm.*M(:,b))]
end 
for c = 1:l
    t2 = [t2, sum(gausgm.*M(:,c))]
end 
